...
def save_result_to_csv(save_path="./result", attack_name="", evaluation_method="",attack_param="",
                       model_name="", result=0):
    file_path = os.path.join(save_path, f"{model_name}.csv")
    ...
    row_name = attack_name + "_" + attack_param

    if evaluation_method not in df.columns:
        df[evaluation_method] = None
    if row_name not in df.index:
        df.loc[row_name] = [None] * len(df.columns)

    df.at[row_name, evaluation_method] = result
    df.to_csv(file_path)
