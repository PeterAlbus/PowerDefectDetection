...
def save_adv_result(adv_xs, adv_labels_numpy, class_num, \
    ...
    if data_type == 'cifar10' or data_type == "cifar100" or save_method == ".npy":
        ...
        for start_idx in range(0, adv_xs.shape[0], batch_size):
            end_idx = min(start_idx + batch_size, adv_xs.shape[0])
            batch_adv_xs = adv_xs[start_idx:end_idx]
            np.save(save_as_black_path + '/{}_{}_advs_{}.npy'.format(attack_method, end_idx, adv_xs.shape[0]), np.array(batch_adv_xs))
            path_adv_xs.append(save_as_black_path + '/{}_{}_advs_{}.npy'.format(attack_method, end_idx, adv_xs.shape[0]))
            path_adv_xs_json = save_as_black_path + '/{}_{}_advs_{}.json'.format(attack_method, end_idx, adv_xs.shape[0])
            save_json(path_adv_xs_json, batch_adv_xs)
            print('saving adv labels...')
            ys_save = []
            for i in range(end_idx - start_idx):
                y = np.zeros((1, class_num), dtype=np.uint8)
                y[0][adv_labels_numpy[start_idx + i]] = 1
                ys_save.append(y[0])
            np.save(save_as_black_path + '/{}_{}_adv_preds_labels_{}.npy'.format(attack_method, end_idx, adv_xs.shape[0]), np.array(ys_save))
            path_adv_ys.append(save_as_black_path + '/{}_{}_adv_preds_labels_{}.npy'.format(attack_method, end_idx, adv_xs.shape[0]))
            path_adv_ys_json = save_as_black_path + '/{}_{}_adv_preds_labels_{}.json'.format(attack_method, end_idx, adv_xs.shape[0])
